...
$exc = new exchange($ecs->table("article_video_cat"), $db, 'cat_id', 'cat_name');
//print_r($exc);exit;
/* act操作项的初始化 */
...
{
    $articlecat = article_video_cat_list(0, 0, false);
    ...
    assign_query_info();
    $smarty->display('articlecat_video_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('articlecat_video_list.htm'));
}
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    assign_query_info();
    $smarty->display('articlecat_video_info.htm');
}
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        $sql = "SELECT cat_type FROM " . $ecs->table('article_video_cat') . " WHERE cat_id = '$_POST[parent_id]'";
    ...
    $sql = "INSERT INTO ".$ecs->table('article_video_cat')."(cat_name, cat_type, cat_desc,keywords, parent_id, sort_order, show_in_nav)
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT cat_id, cat_name, cat_type, cat_desc, show_in_nav, keywords, parent_id,sort_order FROM ".
           $ecs->table('article_video_cat'). " WHERE cat_id='$_REQUEST[id]'";
    $cat = $db->GetRow($sql);
    ...
    $options    =   article_video_cat_list(0, $cat['parent_id'], false);
    $select     =   '';
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $row = $db->getRow("SELECT cat_type, parent_id FROM " . $ecs->table('article_video_cat') . " WHERE cat_id='$_POST[id]'");
    ...
    /* 检查设定的分类的父分类是否合法 */
    $child_cat = article_video_cat_list($_POST['id'], 0, false);
    if (!empty($child_cat))
    {
    ...
    $dat = $db->getOne("SELECT cat_name, show_in_nav FROM ". $ecs->table('article_video_cat') . " WHERE cat_id = '" . $_POST['id'] . "'");
...
{
    check_authz_json('article_video_cat');
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "SELECT cat_type FROM " . $ecs->table('article_video_cat') . " WHERE cat_id = '$id'";
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('article_video_cat') . " WHERE parent_id = '$id'";
    ...
    $url = 'articlecat_video.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
    ...
    if (cat_update($id, array('show_in_nav' => $val)) != false)
    {
        if($val == 1)
        {
            ...
            if(empty($nid))
            {
                ...
                $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $ecs->table('nav') . " WHERE type = 'middle'");
                $vieworder += 2;
                $catname = $db->getOne("SELECT cat_name FROM ". $ecs->table('article_video_cat') . " WHERE cat_id = '$id'");
                $uri = build_uri('article_video_cat', array('acid'=> $id), $_POST['cat_name']);
...
function cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('article_video_cat'), $args, 'update', "cat_id='$cat_id'");
}
?>
