...
/*初始化数据交换对象 */
$exc   = new exchange($ecs->table("article_video"), $db, 'article_id', 'title');
//$image = new cls_image();
...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    $smarty->display('article_video_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('article_video_list.htm'), '',
...
//    $db->query($sql);
    ...
    assign_query_info();
    $smarty->display('article_video_info.htm');
}
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table("article_video")."(title, cat_id, article_type, is_open, author, ".
...
//    $db->query($sql);
    ...
    $link[0]['href'] = 'article_video.php?act=add';

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'article_video.php?act=list';

    admin_log($_POST['title'],'add','article_video');
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    /* 取文章数据 */
    $sql = "SELECT * FROM " .$ecs->table('article_video'). " WHERE article_id='$_REQUEST[id]'";
...
if ($_REQUEST['act'] =='update')
{
    ...
    $sql = "SELECT file_url FROM " . $ecs->table('article_video') . " WHERE article_id = '$_POST[id]'";
    ...
    if ($exc->edit("title='$_POST[title]', cat_id='$_POST[article_cat]', article_type='$_POST[article_type]', is_open='$_POST[is_open]', author='$_POST[author]', author_email='$_POST[author_email]', keywords ='$_POST[keywords]', file_url ='$file_url', open_type='$open_type', content='$_POST[FCKeditor1]', link='$_POST[link_url]', description = '$_POST[description]'", $_POST['id']))
    {
        ...
        admin_log($_POST['title'], 'edit', 'article_video');
...
elseif ($_REQUEST['act'] == 'edit_title')
{
    ...
    else
    {
        ...
        {
            clear_cache_files();
            admin_log($title, 'edit', 'article_video');
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除原来的文件 */
    $sql = "SELECT file_url FROM " . $ecs->table('article_video') . " WHERE article_id = '$id'";
    $old_url = $db->getOne($sql);
    ...
    if ($exc->drop($id))
    {
        ...
        admin_log(addslashes($name),'remove','article_video');
        clear_cache_files();
    }
...
elseif ($_REQUEST['act'] == 'add_link_goods')
{
    ...
    if ($article_id == 0)
    {
        $article_id = $db->getOne('SELECT MAX(article_id)+1 AS article_id FROM ' .$ecs->table('article_video'));
    }
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        if ($_POST['type'] == 'button_remove')
        {
            ...
            /* 删除原来的文件 */
            $sql = "SELECT file_url FROM " . $ecs->table('article_video') .
